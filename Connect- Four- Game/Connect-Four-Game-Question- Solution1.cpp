...
bool check_win() {
	if(row(grid) || column(grid) || diagonals(grid))
		return true;
	else
		return false;
}
...
bool check_tie_player(char mark) {
	...
	if(row(temp) || column(temp) || diagonals(temp))
		return false;
	return true;
}
