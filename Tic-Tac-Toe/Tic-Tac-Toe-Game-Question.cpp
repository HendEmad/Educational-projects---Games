...
bool check_tie_player(char mark) {
    ...
    for(int i = 0; i < N; i++){
        for(int j = 0; j < N; j++){
            ...
            // diagonal_left
            if((grid[0][0] == mark || grid[0][0] == '_') &&
                (grid[N-1][N-1] == mark || grid[N-1][N-1] == '_') &&
                (grid[N/2][N/2]== mark || grid[N/2][N/2] == '_'))
            {
                count_diagonall += 1;
                if(count_diagonall == N)
                    return false;
            ...
            if (((grid[i][j] ==mark || grid[i][j] == '_') &&
                 (grid[j][i])==mark || grid[j][i] == '_') &&
                 (grid[N/2][N/2]==mark || grid[N/2][N/2] == '_'))
            {
                    count_diagonalr+=1;
                    if (count_diagonalr == N)
                        return false;
            }
        }
    }
